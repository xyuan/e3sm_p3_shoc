                      ...
                      ValueType& result)
{
  if (Serialize) {
    ...
    // Broadcast result to all threads by doing sum of one thread's
    // non-0 value and the rest of the 0s.
    Kokkos::Impl::HIPTeamMember::vector_reduce(Kokkos::Sum<ValueType>(local_tmp));
#endif
...
template <>
struct ExeSpaceUtils<Kokkos::Experimental::HIP> {
  using TeamPolicy = Kokkos::TeamPolicy<Kokkos::Experimental::HIP>;
...
#ifdef KOKKOS_ENABLE_HIP
template <typename ValueType>
class TeamUtils<ValueType,Kokkos::Experimental::HIP> : public TeamUtilsCommonBase<ValueType,Kokkos::Experimental::HIP>
{
  using Device = Kokkos::Device<Kokkos::Experimental::HIP, typename Kokkos::Experimental::HIP::memory_space>;
  using flag_type = int; // this appears to be the smallest type that correctly handles atomic operations
  using view_1d = typename KokkosTypes<Device>::view_1d<flag_type>;
  using RandomGenerator = Kokkos::Random_XorShift64_Pool<Kokkos::Experimental::HIP>;
  using rnd_type = typename RandomGenerator::generator_type;
 ...
 public:
  template <typename TeamPolicy>
  TeamUtils(const TeamPolicy& policy, const double& overprov_factor = 1.0) :
    TeamUtilsCommonBase<ValueType,Kokkos::Experimental::HIP>(policy),
    _num_ws_slots(this->_league_size > this->_num_teams
                  ? (overprov_factor * this->_num_teams > this->_league_size ? this->_league_size : overprov_factor * this->_num_teams)
                  : this->_num_teams),
    _need_ws_sharing(this->_league_size > _num_ws_slots),
